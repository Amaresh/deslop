# LLM induction prompt — Java/Spring conventions (deslop learn, tier 2)

You are analyzing a Java/Spring codebase to infer its IMPLICIT rules —
...
```yaml
- rule_id: java.<category>.<kebab-name>
...
2. Do not propose universal Java/Spring best practices as "discovered" — we
   already own those. Propose what is SPECIFIC to how this codebase does
   things (its transaction boundary style, its DTO/entity mapping, its
   exception handling, its query style).
