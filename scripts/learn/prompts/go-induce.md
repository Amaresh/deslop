# LLM induction prompt — Go conventions (deslop learn, tier 2)

You are analyzing a Go codebase to infer its IMPLICIT rules — conventions
the code consistently follows that were never written down. This output
becomes candidate rules for a stopthatslop pack.
...
```yaml
- rule_id: go.<category>.<kebab-name>
...
2. Do not propose universal Go best practices as "discovered" — we already
   own those. Propose what is SPECIFIC to how this codebase does things
   (its error style, its layering, its naming, its IO boundaries).
3. `adoption: high` = you see it followed consistently (>80% of relevant
   sites); `low` = you see it consistently violated (fix-magnet); `mixed` =
   inconsistent.
4. Flag org_specific conventions honestly — they are valuable to the buyer
   but must be labeled.
