...
// goStmtHasContext reports whether launched work is tied to request
// context: printed text contains Context(), .Done(), or ident ctx.
func goStmtHasContext(text string) bool {
	if strings.Contains(text, "Context()") || strings.Contains(text, ".Done()") {
		return true
	}
	for i := 0; i+3 <= len(text); i++ {
		...
		leftOK := i == 0 || !isIdentByte(text[i-1])
		rightOK := i+3 == len(text) || !isIdentByte(text[i+3])
		if leftOK && rightOK {
			return true
		}
	}
...
func analyzeFunc(fi *fileInfo, fd *ast.FuncDecl, fileScope map[string]string) Function {
	...
	ast.Inspect(fd.Body, func(n ast.Node) bool {
		...
		case *ast.GoStmt:
			if x.Call != nil {
				full := exprText(fi, x.Call)
				fn.GoStmts = append(fn.GoStmts, GoStmt{
					Line:        lineOf(fi, x),
					CallSummary: trunc(full),
					HasContext:  goStmtHasContext(full),
				})
			}
			return true
